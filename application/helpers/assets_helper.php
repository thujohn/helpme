...
function other_asset_url($asset_name, $module_name = NULL, $asset_type = NULL)
{
	...
	if ($asset_type != 'uploads'){
		$asset_location = $obj->config->item('assets_url');
	}else{
		$asset_location = $obj->config->item('uploads_url');
	}
...
function css_asset($asset_name, $module_name = NULL, $attributes = array())
{
	...
	return '<link href="'.css_asset_url($asset_name, $module_name).'.css" rel="stylesheet" type="text/css"'.$attribute_str.' />';
}
...
{
	return '<script type="text/javascript" src="'.js_asset_url($asset_name, $module_name).'.js"></script>';
}

function upload_asset($asset_name)
{
	return other_asset_url($asset_name, '', 'uploads');
}
